...
"""
bu araç ile mükemmel wordlistler oluşturabilirsin
"""

global name, __author__,  __version__
name =  'bopscrk.py'
__author__ = 'hackaxx'
__version__ = 'bopscrk 2020'
__status__ = 'çok iyi bir program'
...
def asks():
    while True:
        minLength = input('  {}[?]{} şifrenin minimum büyüklüğü [1] >>> '.format(color.BLUE, color.END))
        if isEmpty(minLength): minLength = 1; break
        else:
            try:
                minLength = int(minLength); break
            except ValueError:
                print('  {}[!]{} minimum değer sayı olmalıdır'.format(color.RED, color.END))
    while True:
        maxLength = input('  {}[?]{} şifrenin maksimum uzunluğu [50] >>> '.format(color.BLUE, color.END))
        if isEmpty(maxLength): maxLength = 50; break
        else:
            try:
                maxLength = int(maxLength)
                if maxLength < minLength: print('  {}[!]{} maksimum değer minimum değerden büyük olmalıdır'.format(color.RED, color.END))
                else: break
            except ValueError:
                print('  {}[!]{} maksimum değer sayı olarak girilmelidir'.format(color.RED, color.END))

    firstname = input('  {}[?]{} hedef adı >>> '.format(color.BLUE, color.END))
    surname = input('  {}[?]{} hedef soyadı >>> '.format(color.BLUE, color.END))
    lastname = input('  {}[?]{} hedefin ikinci adı >>> '.format(color.BLUE, color.END))

    while True:
        birth = input('  {}[?]{} hedef doğum günü (GG/AA/YYYY) >>> '.format(color.BLUE, color.END))
        if not isEmpty(birth) and not is_valid_date(birth):
            print('  {}[!]{} doğum tarihi yanlış girildi'.format(color.RED, color.END))
        else:
            break

    others = input('  {}[?]{} hedefin şifresinde kullanmış olabileceği kelimeler(virgülle ayırabilirsiniz) >>> '.format(color.BLUE, color.END))

    leet = input('  {}[?]{} peki leet yapmak ister misiniz? [E/H] >>> '.format(color.BLUE, color.END))
    case = input('  {}[?]{} case yapmak ister misiniz [E/H] >>> '.format(color.BLUE, color.END))

    if leet.lower() == 'e': leet = True
    else: leet = False

    if case.lower() == 'e': case = True
    else: case = False

    while True:
        nWords = input('  {}[?]{} en çok kaç tane kelimeyi birleştireceksin? [2] >>> '.format(color.BLUE, color.END))
        if isEmpty(nWords): nWords = 2; break
        else:
            try:
                nWords = int(nWords)
                if nWords < 2:
                    print("{}[!]{} birleştirilecek kelimeler 2'den büyük olmalı".format(color.RED, color.END))
                else:
                    break
            except ValueError:
                print('  {}[!]{} birleştirilecek kelimeler sayı olmalı'.format(color.RED, color.END))

    artists = input('  {}[?]{} hedefin sevdiği sanatçılar (virgülle ayrılmış) >>> '.format(color.BLUE, color.END))
    if isEmpty(artists): artists = False

    ly_all_transforms = input('  {}[?]{} hedefin sevdiği sanatçıların şarkı sözleri de wordlist oluşturmada kullanılsın mı?[E/H] >>> '.format(color.BLUE, color.END))
    if ly_all_transforms.lower() == 'e': ly_all_transforms = True
    else: ly_all_transforms = False

    while True:
        exclude = input('  {}[?]{} diğer wordlistlerden kelimelerde içersin mi? >>> '.format(color.BLUE, color.END))
        if isEmpty(exclude): exclude = True; break
        else:
            ...
            for wl_path in exclude:
                if not os.path.isfile(wl_path):
                    valid_paths = False
                    print('  {}[!]{} {} bulunamadı'.format(color.RED, color.END, wl_path))

            if valid_paths:
                break

    outfile = input('  {}[?]{} peki wordlist adı ne olsun [wordlist.txt] >>> '.format(color.BLUE, color.END))
    if isEmpty(outfile): outfile = 'wordlist.txt'
...
def main():
    ...
    if artists:
        try:
            ...
            print('maalesef şarkı sözleri bulunurken bir hata çıktı')
            searchLyrics = False
        ...
        for artist in artists:
            ...
            if searchLyrics:
                print('  {}[*]{} aranıyor {} adlı sanatçının şarkı sözleri ve şarkı adları aranıyor...'.format(color.CYAN, color.END, artist.title()))
                lyfinder = LyricsFinder(artist, False, True)
                lyrics = [s.decode("utf-8") for s in lyfinder.lyrics]
                print('  {}[*] {}{}{} bulundu'.format(color.CYAN, color.GREEN, len(lyrics), color.END))
    ...
    # PRINT RESULTS
    ############################################################################
    print('\n  {}[+]{} harcanan zaman:\t{}'.format(color.GREEN, color.END, total_time))
    print('  {}[+]{} wordlist:\t{}{}{}{}'.format(color.GREEN, color.END, color.BOLD, color.BLUE, outfile, color.END))
